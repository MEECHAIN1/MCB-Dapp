...
const EventLogPage: React.FC = () => {
  ...
  useWatchContractEvent({
    ...
    onLogs(newLogs) {
      const mapped = newLogs.map(l => ({
        ...
        details: `Bot #${(l.args as any).tokenId} synchronized to new signature`,
  ...
  // Watch Staking Events (Assuming standard events exist in full ABI)
  useWatchContractEvent({
    address: ADRS.staking as `0x${string}`,
    abi: MINIMAL_STAKING_ABI,
    eventName: 'stake' as any, // Standard staking ABI often has Stake
    onLogs(newLogs) {
      const mapped = newLogs.map(l => ({
        id: l.transactionHash + l.logIndex,
        type: 'STAKE' as const,
        address: (l.args as any).user,
        details: `Energy sacrifice of ${(l.args as any).amount} MEE detected`,
        timestamp: new Date().toLocaleTimeString(),
        txHash: l.transactionHash || ''
      }));
      setLogs(prev => [...mapped, ...prev].slice(0, 50));
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-10">
      <header className="flex items-center justify-between">
        <div>
          <h1 className="text-5xl font-black italic uppercase tracking-tighter">Ritual Logs</h1>
          <p className="text-zinc-500 font-mono text-[10px] uppercase tracking-[0.4em] mt-2">Historical data transmission from the MeeChain core</p>
        </div>
        <div className="flex items-center gap-3 bg-zinc-900 px-6 py-3 rounded-2xl border border-zinc-800">
          ...
          <span className="text-xs font-mono uppercase tracking-widest text-zinc-300">Live Feed Active</span>
        </div>
      </header>
      ...
      <div className="bg-zinc-950 border border-zinc-800 rounded-[2rem] overflow-hidden shadow-2xl min-h-[600px] flex flex-col">
        <div className="grid grid-cols-12 bg-zinc-900/50 p-4 border-b border-zinc-800 text-[10px] font-mono text-zinc-500 uppercase tracking-widest font-bold">
          ...
          <div className="col-span-2">Ritual Type</div>
          <div className="col-span-5">Transmission Details</div>
          <div className="col-span-3 text-right">Reference Hash</div>
        </div>
          ...
          <AnimatePresence initial={false}>
            {logs.length > 0 ? (
              logs.map((log) => (
                ...
                >
                  ...
                  <div className="col-span-2 flex items-center gap-2">
                    {getIcon(log.type)}
                    <span className="text-[10px] font-black uppercase tracking-wider">{log.type}</span>
                  </div>
                  <div className="col-span-5 text-sm font-medium text-zinc-300 group-hover:text-white transition-colors">
                    {log.details}
                  </div>
              ...
              <div className="h-full flex flex-col items-center justify-center py-40 text-center space-y-6 opacity-30">
                ...
                <div>
                  <p className="text-zinc-500 font-mono text-xs uppercase tracking-[0.3em]">Synchronizing with MeeChain core...</p>
                  <p className="text-[10px] text-zinc-700 font-mono uppercase tracking-[0.2em] mt-2">Awaiting the next ritual transmission</p>
                </div>
              </div>
            )}
          </AnimatePresence>
        ...
        <div className="p-4 bg-zinc-900/30 border-t border-zinc-800 flex justify-between items-center">
          ...
          <span className="text-[8px] font-mono text-zinc-600 uppercase tracking-[0.5em]">System Secure • End of Feed</span>
        </div>
      </div>
    </div>
  );
