...
import { Wallet, LogOut, ShieldCheck, Zap } from 'lucide-react';
        ...
        <div className="flex items-center gap-4">
          {isConnected ? (
            <div className="flex items-center gap-2 bg-zinc-900 border border-zinc-700 rounded-full px-4 py-2">
              ...
              <button onClick={() => disconnect()} className="ml-2 hover:text-red-500">
                <LogOut size={14} />
              </button>
            </div>
          ) : (
            <button 
              ...
              className="bg-yellow-500 text-black px-5 py-2 rounded-full font-bold text-xs uppercase tracking-widest hover:bg-yellow-400 transition-all flex items-center gap-2"
            >
              <Wallet size={16} />
              Connect Ritual
            </button>
